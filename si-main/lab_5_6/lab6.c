...
t_ls * push_front(t_ls *lst, char *im, char *fam, char *god, char *score)  /*Создание следующего элемента*/
{
  t_ls *temp, *p;
  temp = (t_ls*)malloc(sizeof(t_ls));
  p = lst -> next;
  lst -> next = temp;
  strcpy(temp -> im, im);
  strcpy(temp -> fam,fam);
...
int main() {
  ...
  printf("Выберите оценку для отбора\n");
  printf("1. 5\n");
  printf("2. 4\n");
  printf("3. 3\n");
  printf("4. 2\n");
  scanf("%s", napr);
  printf("Результаты отбора скопированы vybor.txt\n", napr);
